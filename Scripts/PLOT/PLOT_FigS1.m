...
m_contourf(lonT,latT,topoT,[0 0],'facecolor',[.9 .9 .9],'edgecolor','none');
k=1;
% Drifters for which we remove a part (after gap) in the Labrador sea
for i=1:length(drifter_data_SVP.IdBuoy)
    if sum(drifter_data_SVP.gap(:,i))>0
        if sum(inpolygon(drifter_data_SVP.Longitude(drifter_data_SVP.gap(:,i)==1,i),drifter_data_SVP.Latitude(drifter_data_SVP.gap(:,i)==1,i),area_full(1,:),area_full(2,:)))>0
            m_plot(drifter_data_SVP.Longitude(drifter_data_SVP.gap(:,i)==0,i),drifter_data_SVP.Latitude(drifter_data_SVP.gap(:,i)==0,i),'color',[.7 .7 .7],'linewidth',0.5);
            m_plot(drifter_data_SVP.Longitude(drifter_data_SVP.gap(:,i)==1,i),drifter_data_SVP.Latitude(drifter_data_SVP.gap(:,i)==1,i),'color',[.7 .7 .7],'linewidth',0.5, 'linestyle',':');
        end
    end
end
...
for i=1:length(drifter_data_SVP.IdBuoy)
    if sum(drifter_data_SVP.gap(:,i))>0
        if sum(inpolygon(drifter_data_SVP.Longitude(drifter_data_SVP.gap(:,i)==1,i),drifter_data_SVP.Latitude(drifter_data_SVP.gap(:,i)==1,i),area_LSh(1,:),area_LSh(2,:)))>0
            m_plot(drifter_data_SVP.Longitude(drifter_data_SVP.gap(:,i)==0,i),drifter_data_SVP.Latitude(drifter_data_SVP.gap(:,i)==0,i),'color',color(k,:),'linewidth',1.5);
            m_plot(drifter_data_SVP.Longitude(drifter_data_SVP.gap(:,i)==1,i),drifter_data_SVP.Latitude(drifter_data_SVP.gap(:,i)==1,i),'color',color(k,:),'linewidth',1.5, 'linestyle',':');
            k=k+1;
            i;
        end
    end
