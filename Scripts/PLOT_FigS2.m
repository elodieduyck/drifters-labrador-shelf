...
for i=1:length(drift_sel_HS)
    % if drift_lon(outdate_save_HS(i),drift_sel_HS(i))>-66 % roughly the upper border of the box, just making sure we're looking at  drifters exiting south, not north
    % alternatively check if south or west of entrance (nom more confuHSng)
    % if drift_lat(outdate_save_HS(i),drift_sel_HS(i))<drift_lat(indate_save_HS(i),drift_sel_HS(i)) |  drift_lon(outdate_save_HS(i),drift_sel_HS(i))<drift_lon(indate_save_HS(i),drift_sel_HS(i))
    try
        m_plot(drift_lon(:,drift_sel_HS(i)),drift_lat(:,drift_sel_HS(i)),'Color',[.7 .7 .7])
        % [x,y]=m_ll2xy(drift_lon(outdate_save_HS(i):outdate_save_HS(i)+360,drift_sel_HS(i)),drift_lat(outdate_save_HS(i):outdate_save_HS(i)+360,drift_sel_HS(i)));
